...
<template>
  <nav
    ...
    style="background: rgba(51,48,43,.88);"
  >
    <!-- Hamburguer -->
    <button
      type="button"
    ...
    <div class="flex items-center gap-2.5 font-display text-lg font-bold tracking-wide uppercase">
      <span class="flex h-7 w-7 items-center justify-center rounded-md bg-gradient-to-br from-calipso to-verde text-[15px] text-primary-ink shadow-[0_0_16px_rgba(63,174,154,.4)]">▲</span>
      <span>{{ title }}</span>
    ...
    <div v-if="userName || $slots['user-menu']" class="relative">
      ...
      >
        <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-calipso to-verde font-mono text-xs font-bold text-primary-ink">{{ iniciales(userName) }}</span>
      ...
      <Transition name="dropdown">
        ...
        >
          <slot name="user-menu" :cerrar="() => { menuAbierto = false }" />
        </div>
      </Transition>
