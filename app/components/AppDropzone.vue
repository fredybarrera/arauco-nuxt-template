...
<template>
  <div>
    ...
    <div v-if="model.length" class="mt-3.5 flex flex-col gap-2">
      <div v-for="file in model" :key="file.id" class="flex items-center gap-2.5 py-2.5 px-3 bg-paper-2 rounded-sm text-[13px]">
        <AppIcon name="file" :size="16" class="text-ink-soft shrink-0" />
        <span class="flex-1 font-semibold text-ink overflow-hidden text-ellipsis whitespace-nowrap">{{ file.name }}</span>
